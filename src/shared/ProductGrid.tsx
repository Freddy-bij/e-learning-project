...
const ProductCart = ({ product }: ProductCartProps) => {
  ...
  return (
    <div 
      ...
      className="group relative bg-white transition-all duration-300 hover:shadow-lg p-2 rounded-sm cursor-pointer"
    >
      ...
      <div className="relative overflow-hidden aspect-[3/4] mb-3">
        ...
        <div className="absolute bottom-0 left-0 w-full transform translate-y-full group-hover:translate-y-0 transition-transform duration-300 flex items-center bg-blue-600 z-20">
          <button 
            ...
            className="flex-1 flex items-center justify-center gap-2 py-2.5 text-white font-bold text-[10px] uppercase tracking-wider"
          >
          ...
          <button 
            ...
            className="p-2.5 border-l border-blue-500 text-white hover:bg-blue-700"
          >
            <Search size={16} />
          </button>
        </div>
              ...
              <div className="flex gap-3">
                {product.sizes.map((size) => (
                  <span 
                    key={size} 
                    onClick={(e) => e.stopPropagation()} 
                    className="text-[10px] text-gray-500 font-bold uppercase hover:text-blue-600 cursor-pointer"
                  >
                    {size}
                  </span>
                ))}
